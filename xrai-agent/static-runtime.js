(()=>{
  ...
  async function resolveBranch(repo){
    ...
    for(const ref of ['main','master','develop','dev','trunk','gh-pages']){try{await flat(repo,ref);branchCache.set(repo,ref);return ref}catch{}}
    throw new Error(`Could not resolve a public branch for ${repo} after GitHub rate limiting.`);
  }
  async function fallbackGitHub(url){
    const u=new URL(url),parts=u.pathname.split('/').filter(Boolean);if(parts[0]!=='repos'||parts.length<3)return null;
    const repoParts=splitRepo(`${parts[1]}/${parts[2]}`);if(!repoParts)return null;const repo=repoParts.join('/');
    if(parts.length===3){const branch=await resolveBranch(repo),name=repoParts[1];return response({private:false,default_branch:branch,name,full_name:repo,html_url:`https://github.com/${repo}`,xrai_mirror:'jsdelivr'})}
    if(parts[3]==='commits'&&parts[4]){const ref=decodeURIComponent(parts[4]);await flat(repo,ref);return response({sha:ref,xrai_mirror:'jsdelivr'})}
    if(parts[3]==='git'&&parts[4]==='trees'&&parts[5]){const ref=decodeURIComponent(parts[5]),data=await flat(repo,ref),tree=(data.files||[]).filter(f=>f?.name&&f.type!=='directory').map(f=>({path:String(f.name).replace(/^\//,''),type:'blob',size:Number(f.size||0),sha:String(f.hash||'')}));return response({sha:ref,truncated:false,tree,xrai_mirror:'jsdelivr'})}
    return null;
  }
