...
page.on('response',r=>{const u=r.url(),optionalSearch=/search\.jina\.ai/i.test(u);if(!optionalSearch&&r.status()>=400&&/jt5d\.github\.io\/xrai-agent|api\.github\.com|raw\.githubusercontent\.com|webcontainer|jsdelivr|wikipedia|algolia|duckduckgo/i.test(u))consoleErrors.push(`HTTP ${r.status()} ${u}`)});
