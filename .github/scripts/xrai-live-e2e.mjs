...
page.on('response',r=>{const u=r.url(),expectedRateLimit=/api\.github\.com\/repos\/JT5D\/xrai-agent/i.test(u)&&[403,429].includes(r.status()),optionalSearch=/search\.jina\.ai/i.test(u);if(!expectedRateLimit&&!optionalSearch&&r.status()>=400&&/jt5d\.github\.io\/xrai-agent|api\.github\.com|raw\.githubusercontent\.com|webcontainer|jsdelivr|wikipedia|algolia|duckduckgo/i.test(u))consoleErrors.push(`HTTP ${r.status()} ${u}`)});
