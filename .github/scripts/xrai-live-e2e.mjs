...
const EXPECTED_VERSION=process.env.XRAI_VERSION||'0.3.4';
...
const consoleErrors=[],badRequests=[],githubRequests=[],mirrorRequests=[];
page.on('console',m=>{if(m.type()==='error')consoleErrors.push(m.text())});
page.on('pageerror',e=>consoleErrors.push(`pageerror: ${e.message}`));
page.on('request',r=>{const u=r.url();if(u.includes('api.github.com/repos/'))githubRequests.push(u);if(u.includes('data.jsdelivr.com/v1/package/gh/'))mirrorRequests.push(u);if(/api\.github\.com\/repos\/(filesystem\/repository|api\.github\.com\/repos)/i.test(u))badRequests.push(u)});
page.on('response',r=>{const u=r.url(),expectedRateLimit=/api\.github\.com\/repos\/JT5D\/xrai-agent/i.test(u)&&[403,429].includes(r.status());if(!expectedRateLimit&&r.status()>=400&&/jt5d\.github\.io\/xrai-agent|api\.github\.com|raw\.githubusercontent\.com|webcontainer|jsdelivr/i.test(u))consoleErrors.push(`HTTP ${r.status()} ${u}`)});
...
async function stabilize(){await waitForApp();for(let i=0;i<4;i++){if(await page.evaluate(()=>Boolean(navigator.serviceWorker?.controller&&globalThis.crossOriginIsolated))){await page.waitForTimeout(1000);return}await page.waitForTimeout(1000);await page.reload({waitUntil:'domcontentloaded'});await waitForApp()}throw new Error('Pages never became service-worker controlled and cross-origin isolated.')}
...
try{
  ...
  // Force anonymous GitHub REST rate limiting so the production mirror fallback is exercised on every run.
  await page.route(/https:\/\/api\.github\.com\/repos\/JT5D\/xrai-agent(?:\/.*)?$/,route=>route.fulfill({status:403,contentType:'application/json',body:'{"message":"API rate limit exceeded"}'}));
  await submit('review repo, fix any failed tests, verify & explain');await page.waitForFunction(()=>document.querySelector('#currentTask')?.textContent?.includes('review repo'),null,{timeout:10000});await waitForRunDone();await screenshot('02-repo-run-complete');
  const body=await page.locator('body').innerText();if(badRequests.length)throw new Error(`Bogus GitHub requests: ${badRequests.join(', ')}`);if(!githubRequests.some(u=>/api\.github\.com\/repos\/JT5D\/xrai-agent/i.test(u)))throw new Error('No real JT5D/xrai-agent request observed.');if(!mirrorRequests.length)throw new Error('Rate-limit fallback did not use the jsDelivr public repo mirror.');if(!/verified successfully/i.test(body)||!/PASS npm test/i.test(body)||!/PASS npm run check/i.test(body))throw new Error('Repo execution did not render complete real verification evidence.');

  const before=await page.locator('#messages').innerText();await page.reload({waitUntil:'domcontentloaded'});await stabilize();const after=await page.locator('#messages').innerText();if(!after.includes('review repo')||!after.includes('verified successfully')||after.length<Math.min(120,before.length/2))throw new Error('Completed result did not survive reload.');await screenshot('03-after-reload');

  const githubCount=githubRequests.length;await submit('is it fixed?');await page.waitForTimeout(1500);const userTail=(await page.locator('.msg.user .bubble p').allTextContents()).at(-1)||'';if(!userTail.includes('Previous XRAI context')||!userTail.includes('Task: review repo')||!userTail.includes('Result:')||userTail.includes('No completed result was recorded'))throw new Error('Follow-up did not receive prior run context.');if(githubRequests.length!==githubCount)throw new Error('Vague follow-up incorrectly started another repo execution.');if(badRequests.length)throw new Error(`Bogus GitHub requests after follow-up: ${badRequests.join(', ')}`);await screenshot('04-followup-context');

  const errors=consoleErrors.filter(x=>!/favicon\.ico/i.test(x));await fs.writeFile(`${outDir}/console-errors.txt`,errors.join('\n'));await fs.writeFile(`${outDir}/github-requests.txt`,githubRequests.join('\n'));await fs.writeFile(`${outDir}/mirror-requests.txt`,mirrorRequests.join('\n'));if(errors.length)throw new Error(`Relevant browser/network errors:\n${errors.join('\n')}`);console.log('XRAI live E2E PASS');console.log(`GitHub requests: ${githubRequests.length}; mirror requests: ${mirrorRequests.length}`);
